...
void implanting_cycle(unsigned int i){
	...
	if(position.size() != 0){
        ...
        if (BED_FILE.size() == 0){
			...
			implanting_class (jaspar_map, multifasta_map,i);
			if(position_vector.size() > 1){	
				//checking if the implanting positions don't bring to any overlap
				check_overlapping(jaspar_map);
			}
...
void debug_map(map<unsigned int, string> multifasta_map, map<unsigned int, string>){
	map<unsigned int, string>::iterator itr;
	ofstream outfile1;
	outfile1.open("BED_map.txt");
    outfile1 << "The map is : \n\n";
    outfile1 << "KEY\tELEMENT\n";
    for (itr = multiBED_map.begin(); itr != multiBED_map.end(); ++itr) {
    	outfile1 << itr->first<< '\t' << itr->second << '\n';
    }
    cout << endl;
	outfile1.close();
	map<unsigned int, string>::iterator iter;
	ofstream outfile;
	outfile.open("multi_map.txt");
 	outfile << "The map is : \n\n";
 	outfile << "KEY\tELEMENT\n";
 	for (iter = multifasta_map.begin(); iter != multifasta_map.end(); ++iter) {
