  ...
  if (MFASTA_FILE.empty()) {
    ...
    for (unsigned int i = 0; i < kmers_vector.size(); i++) {
      ...
      for (unsigned int j = 0; j < len[i]; j++) {
        ...
        //for (unsigned int x = 0; x < P_vector[0].oligo.size(); x++) {
        ...
        if(H.freq1 >= freq_treshold){
            z_test_class Z(H.PWM_hamming, C.GEP, j + 1, kmers_vector);
...
// Checking function to control if, for any k-mers inserted as input, there is a
// distance parameter
void map_class::check_kmer_dist() {
  ...
  if (kmers_vector.size() != distance_vector.size()) {
    ...
    cerr << "\nERROR: Please insert an equal number of k-mers and distance "
            "parameters!\n";
    display_help();
    exit(1);
  }
  for (unsigned int i = 0; i < kmers_vector.size(); i++) {
    if (distance_vector[i] > kmers_vector[i]) {
      cerr << "\nERROR: Please distance parmater must be lower or equal to "
              "k-mers length!\n";
      display_help();
      exit(1);
    }
...
void ZetaClass::z_score_parameters_calculation(vector<double> all_local_scores, vector<double> all_global_scores) {
  // PROFILE_FUNCTION();
  double local_sum =
      accumulate(all_local_scores.begin(), all_local_scores.end(), 0.0);
  double global_sum =
      accumulate(all_global_scores.begin(), all_global_scores.end(), 0.0);
  cout << "Global and local sum: " << global_sum << "\t" << local_sum << endl;
...
void ZetaClass::z_score_calculation(vector<double> all_local_scores) {
  // PROFILE_FUNCTION();
...
vector<double> BestStrandOligo(vector<double> oligo_scores_horizontal_FWD, 
  ...
  return oligo_scores_horizontal_BEST;
}
...
void HammingClass::Freq1Calc() {
  // cout << vert_vector[0] << " " << tot_freq << endl;
...
void SeedClass::FillSeed(vector<PvalueClass> &P_vector) {
  ...
  for (unsigned int i = 0; i < P_vector.size() && i < secondary; i++) {
    if (ordering == "p") {
      ...
      cout << "Seed vector size pval: " << seed_vector.size() << endl;
    } else {
      ...
      cout << "Seed vector size vertical: " << seed_vector.size() << endl;
    }
  }
}
...
// Function to create a map of oligos + their occurrences along all the
// sequences
void map_class::table_creation_horizontal(vector<bed_class> &GEP) {
  ...
  for (unsigned int k = 0; k < kmers_vector.size(); k++) {
    ...
    for (unsigned int j = 0; j < GEP.size(); j++) {
      ...
      // Extracted and analyzed all words of length k that are found by
      // scrolling through the sequence
      for (unsigned int i = 0;
           i < (GEP[j].sequence.size() - kmers_vector[k] + 1); i++) {
...
void map_class::or_ver_kmer_count(string bases,
                                  ...
                                  unordered_map<string, unsigned int> &minus) {
  // PROFILE_FUNCTION();
  ...
  // If the oligo is not present yet --> insert it into the map with an
  // occurrences value of 1
  else {
...
void map_class::table_creation_vertical(vector<bed_class> &GEP) {
  ...
  if (MFASTA_FILE.empty()) {
    ...
    // RAM_usage();
    cout << "- [5] Counting all k-mers positional occurrences and making "
            "vertical maps  \n";
  }
  ...
  for (unsigned int k = 0; k < kmers_vector.size(); k++) {
    ...
    // vector to store the total number of possible oligo per position, useful
    // to calculate frequences
    vector<unsigned int> tot_freq_vec;

    // Extracted and analyzed the oligo in position "i"
    for (unsigned int i = 0; i < (GEP[0].sequence.size() - kmers_vector[k] + 1);
         i++) {

      unsigned int tot_freq = 0;

      // Make the analysis of all the sequences' oligo in position "i" (vertical
      // analysis)
      for (unsigned int j = 0; j < GEP.size(); j++) {
      ...
      if (DS) {
        ...
        // one who has the most occurrences in FWD strand)
        select_best(vertical_plus);
      }

      // The maps just created is saved into a vector of maps (one for each
      // position)
    ...
    // The vector of maps created is stored into a vector to create a matrix in
    // which on the dimension 1 we have the k-mers used and on dimension 2 the
    // positions on sequences
    vector_kmers_maps_plus.emplace_back(maps_vector_positions_plus);

    // Also the all possible oligos to calculate frequences are stored in a
    // matrix (k-mers x position)
...
// Function to count oligo occurrences and to create and fill the maps. It also
// count all the total oligo present in position (taking into account to the
// palindrome oligos) --> this count will be useful to calculate the frequency
void map_class::vertical_kmer_count(
    string bases,
    map<pair<string, string>, pair<unsigned int, unsigned int>> &plus,
    unsigned int &tot_freq) {
  // PROFILE_FUNCTION();
  map<pair<string, string>, pair<unsigned int, unsigned int>>::iterator it_plus;
  map<pair<string, string>, pair<unsigned int, unsigned int>>::iterator
      it_plus_rev;
  ...
  // If analysis is on SS --> possible oligos per position ++1 (it will be equal
  ...
  // If the pair oligo + RC is already present in the plus map --> increase
  // occurrences by 1
  if (it_plus != plus.end()) {
  ...
  // Else If the pair RC + oligo is already present in the plus map --> increase
  // occurrences by 1
...
void map_class::select_best(
  ...
  for (map<pair<string, string>, pair<unsigned int, unsigned int>>::const_iterator
    ...
    // Else if Oligo occurrences are higher then RC occurrences is FWD strand
      ...
      copy.insert({{it->first.first, it->first.second},
                   {it->second.first, it->second.second}});
    }
  }

  // Clearing the map vertical plus to be substituted by "copy", which contains
  // only the best pairs
  vertical_plus.clear();
  vertical_plus = copy;
  copy.clear();
...
// Function to create a matrix of p_value class (1 dimension = different k-mers,
...
void map_class::P_VALUE_MATRIX_creation() {
  ...
  for (unsigned int j = 0; j < vector_kmers_maps_plus.size(); j++) {
    ...
    // Sums vector of the first N oligo occurrences saved into a matrix and then
    // is cleaned up
...
void map_class::HAMMING_MATRIX_creation() {
  ...
  if (MFASTA_FILE.empty()) {
    ...
    // RAM_usage();
    cout << "- [7] Calculating best oligos hamming neighbours and filling "
            "Hamming matrix  \n";
  }
  ...
  for (unsigned int j = 0; j < P_VALUE_MATRIX.size(); j++) {
    ...
    for (unsigned int i = 0; i < P_VALUE_MATRIX[j].size(); i++) {
      ...
      // passed as input
        ...
        // cout << "Before HAMMING_MATRIX.emplace_back(HAMMING_VECTOR);"<< endl;
...
void map_class::Z_TEST_MATRIX_creation(vector<bed_class> &GEP) {
  ...
  if (MFASTA_FILE.empty()) {
    ...
    // RAM_usage();
    cout << "- [9] Calculating Z-test from PWM matrices shifing and filling "
            "Z-test matrix  \n";
  }
  // For every kmer in input
  for (unsigned int i = 0; i < HAMMING_MATRIX.size(); i++) {
    ...
    for (unsigned int j = 0; j < HAMMING_MATRIX[i].size(); j++) {
      ...
      if (local_maxima_grouping == false) {
        ...
        // if the position is not the first -> return the pos-1 freq_1 -> else
        // the pos-1 freq_1 value remain 0
        if (j != 0) {
          Frequence_1_prev = HAMMING_MATRIX[i][j - 1].FREQUENCE_1;
        }

        // if the position is not the last -> return the pos+1 freq_1 -> else
        // the pos+1 freq_1 value remain 0
        ...
        local_max = find_local_max(HAMMING_MATRIX[i][j].FREQUENCE_1,
                                   Frequence_1_prev, Frequence_1_post);
      }
...
// Return 1 only if the frequence in position is higher than frequences in pos-1
// and in pos+1
...
void p_value_class::multimap_creation(
...
// Finding N2 parameters thanks to the accumulate function, which allows to sum
...
void p_value_class::filling_KNT_vectors(
  ...
  for (multimap<pair<unsigned int, unsigned int>,
                pair<string, string>>::reverse_iterator it_rev =
           vertical_multimap.rbegin();
    ...
    // Using the total_oligo_N2 (calculated before in N2_calculation function)
    // defining the N2 value as definition (Total number of k-mers - N1)
...
// Using gsl library hypergeometric_Q function and the parameters found before
// (K,N1,N2,T) calculate each oligo p_value and store it into a vector
void p_value_class::calculating_p_value() {
  // PROFILE_FUNCTION();
  for (unsigned int i = 0; i < K_vec.size(); i++) {
    // cout << "K: " << K_vec[i] << "\tN1: " << N1_vec[i] 
    // << "\tN2: " << N2_vec[i] << "\tT: " << T << endl;
    double p_value =
        gsl_cdf_hypergeometric_Q(K_vec[i], N1_vec[i], N2_vec[i], T);

    // Checking if p_value is too low and therefore is automatically rounded to
    // 0
...
// Function to sort all the oligos in each position following the lowest p_value
// scores
void p_value_class::sorting_p_value() {
  ...
  // 1. p_value_sort = a multimap containing p_value and the pair oligo +
  // oligo's RC to order oligos by lowest p_value
  // 2. p_value_KNT = a multimap composed by p_value at first position to
  // guarantee the order and oligo + K,N1,N2,T vector at second position
  for (multimap<pair<unsigned int, unsigned int>,
                pair<string, string>>::reverse_iterator it_rev =
           vertical_multimap.rbegin();
       it_rev != vertical_multimap.rend(); it_rev++, i++) {
    ...
    p_value_sort.insert(
        {p_value_vec[i], {it_rev->second.first, it_rev->second.second}});
    p_value_KNT.insert({p_value_vec[i], {it_rev->second.first, KNT}});

    // Clear the KNT vector to mantain its size (need to be formed by 4
    // elements(K,N1,N2,T))
    KNT.clear();
  }
}

// Check and select if oligos are going to be printed in output file ordered by
// occurrences or p_values
void p_value_class::checking_ordering(
    map<pair<string, string>, pair<unsigned int, unsigned int>> &pair_map,
  ...
  if (ordering == "p") {
    ...
    if (DS) {
      ...
      print_debug_occurrences_SS(pair_map, position, outfile, freq,
                                 p_value_vec);
    }
  }
}
...
hamming_class::creating_sum_occurrences_multimap(
  ...
  for (multimap<pair<unsigned int, unsigned int>,
                pair<string, string>>::reverse_iterator rev_it =
           pair_map.rbegin();
       rev_it != pair_map.rend(); rev_it++) {

    sum_occurrences_multimap.insert(
        {(rev_it->first.first + rev_it->first.second),
         make_pair(rev_it->second.first, rev_it->second.second)});
  }

  return sum_occurrences_multimap;
}

// Scrolling the vertical positional multimap find the best oligo for
// occurrences. If more than one is present selecting the oligo which has more
// hamming neighbours.
void hamming_class::find_best_oligos(
 ...
 // PROFILE_FUNCTION();
  ...
  multimap<pair<unsigned int, unsigned int>,
           pair<string, string>>::reverse_iterator it_rev_SS =
      vertical_multimap.rbegin();
...
void hamming_class::checking_best_oligo(
    unsigned int distance,
    multimap<pair<unsigned int, unsigned int>, pair<string, string>>
        &vertical_multimap) {
  // PROFILE_FUNCTION();
...
// Function for find the real_best_oligo (selecting the one who has more
// neighbours than the others) if more than one best oligo has been found
string hamming_class::select_real_best_oligo(
    unsigned int distance,
    multimap<pair<unsigned int, unsigned int>, pair<string, string>>
        &vertical_multimap) {
  // PROFILE_FUNCTION();
  ...
  for (unsigned int i = 0; i < best_oligos.size(); i++) {
    ...
    // If the analysis is on the first best oligo -> the max similarity (that is
    // the max number of neighbour) corresponds to its similar_oligos number
    if (i == 0) {
    ...
    // If an oligo has a number of neighbour (similar_oligo.size()) which
    // exceeds the best partial so far (max_similarity) -> its neighbour number
    // becames the best and the index of oligo (into the best_oligos vector) is
    // saved.
    ...
    // And also similar_oligos_occurrences vector needs to be cleaned up to
    // avoid subsequently interferences (because the function
    // find_distanced_oligos calculates also the neighbours occurrences and
    // saves them into similar_oligo_occurrences vector)
...
void hamming_class::find_secondary_hamming(
  ...
  for (unsigned int neighbour = 0; neighbour < number_first_hamming;
       neighbour++) {
...
void hamming_class::find_distanced_oligos(
    string best, unsigned int distance,
    multimap<pair<unsigned int, unsigned int>, pair<string, string>>
        &vertical_multimap) {
  // PROFILE_FUNCTION();
  bool is_similar;
  ...
  for (multimap<pair<unsigned int, unsigned int>,
    ...
    // If the oligo in analysis is not the real_best_oligo (to avoid the
    // comparison to himself and to avoid to be added to similar oligos vector
    // in case of secondary hamming)
    if (it_rev->second.first != best &&
        it_rev->second.first != real_best_oligo) {

      // Call the function is_similar_oligo to compare the real_best_oligo to
      // the current oligo. The function returns 1 if it is, otherwise 0
...
// Function to compare two oligos and find out how many charachters are apart
// (distanced)
bool hamming_class::is_similar_oligo(string oligo_1, string oligo_2,
                                     unsigned int distance) {
  // PROFILE_FUNCTION();
...
// Function to calculate the frequence_1 (total of similar occurrences / total
// of possible oligos in the position)
...
// Starting from the best oligo and his hamming neighbours the function builds a
// PWM_matrices following their sequences and occurrences
void hamming_class::PWM_hamming_creation() {
  ...
  // For each bases (position) of oligo
  for (unsigned int character = 0; character < similar_oligos[0].size();
       character++) {
    ...
    for (unsigned int oligo = 0; oligo < similar_oligos.size(); oligo++) {
      ...
      similar_oligos_map.insert(pair<string, double>(
          similar_oligos[oligo], similar_oligos_occurrences[oligo]));

      switch (similar_oligos[oligo][character]) {

      // Increment base counters of the oligo occurrences
      case 'A':
        counter_A += similar_oligos_occurrences[oligo];
        break;
      case 'C':
        counter_C += similar_oligos_occurrences[oligo];
        break;
      case 'G':
        counter_G += similar_oligos_occurrences[oligo];
        break;
      case 'T':
        counter_T += similar_oligos_occurrences[oligo];
        break;
      }
    }
...
bool hamming_class::EM_convergence(vector<vector<double>> &PWM_old,
...
// If the analysis is on Double Strand and oligos need to be ordered by p_value
// this is the function which writes on the output file
void p_value_class::print_debug_p_value_DS(
    map<pair<string, string>, pair<unsigned int, unsigned int>> &pair_map,
    unsigned int position, ofstream &outfile, unsigned int freq) {
  // PROFILE_FUNCTION();
  unsigned int c = 0;
  ...
  // Scrollig the p_value_sort multimap from first element to -nth element
  // assign the rigth value to each feature and print them
  for (multimap<double, pair<string, string>>::iterator it_pair =
           p_value_sort.begin();
       it_pair != p_value_sort.end() && c < top_N; it_pair++, c++) {
    ...
    outfile << Oligo << "\t" << Num_Occ_FWD << "\t" << Num_Occ_REV << "\t"
            << Sum_Occ_Oligo << "\t";
...
// If the analysis is on Single Strand and oligos need to be ordered by p_value
// this is the function which writes on the output file
void p_value_class::print_debug_p_value_SS(
    map<pair<string, string>, pair<unsigned int, unsigned int>> &pair_map,
    unsigned int position, ofstream &outfile, unsigned int freq) {
  // PROFILE_FUNCTION();
  unsigned int c = 0;
...
// If the analysis is on Double Strand and oligos need to be ordered by
// occurrences this is the function which writes on the output file
void p_value_class::print_debug_occurrences_DS(
    map<pair<string, string>, pair<unsigned int, unsigned int>> &pair_map,
    unsigned int position, ofstream &outfile, unsigned int freq,
  ...
  // Scrollig the p_value_sort multimap from first element to -nth element
  // assign the rigth value to each feature and print them
  for (multimap<unsigned int, pair<string, string>>::reverse_iterator it_rev =
           sum_occurrences_multimap.rbegin();
       it_rev != sum_occurrences_multimap.rend() && c < top_N; it_rev++, c++) {

    pair_map_it =
        pair_map.find(make_pair(it_rev->second.first, it_rev->second.second));
...
multimap<unsigned int, pair<string, string>>
p_value_class::creating_sum_occurrences_multimap(
    multimap<pair<unsigned int, unsigned int>, pair<string, string>>
        &pair_map) {
...
// If the analysis is on Single Strand and oligos need to be ordered by
// occurrences this is the function which writes on the output file
void p_value_class::print_debug_occurrences_SS(
    map<pair<string, string>, pair<unsigned int, unsigned int>> &pair_map,
    unsigned int position, ofstream &outfile, unsigned int freq,
    vector<double> p_value_vec) {
  // PROFILE_FUNCTION();
  ...
  for (multimap<pair<unsigned int, unsigned int>,
                pair<string, string>>::reverse_iterator it_rev =
           vertical_multimap.rbegin();
       it_rev != vertical_multimap.rend() && c < top_N; it_rev++, c++) {
...
// Function to write the correct header in output positional file, following
// analysis parameters
  ...
  if (DS) {
    ...
    if (ordering == "p") {
      ...
      outfile.open(to_string(kmers_vector[j]) +
                   "-mers_positional_occurrences_" + alias_file +
                   "DS_p_val.txt");
    } else {

      outfile.open(to_string(kmers_vector[j]) +
                   "-mers_positional_occurrences_" + alias_file + "DS_occ.txt");
    }

    outfile << "#Maps vector with kmers occurences (Double Strand) counted for "
               "positions in sequence (for k = "
            << kmers_vector[j] << "):\n";
    outfile << "#Position"
            << "\t"
            << "Rank"
    ...
    if (ordering == "p") {
      ...
      outfile.open(to_string(kmers_vector[j]) +
                   "-mers_positional_occurrences_" + alias_file + "SS_occ.txt");
    }

    outfile << "#Maps vector with kmers occurences (Single Strand) counted for "
               "positions in sequence (for k = "
            << kmers_vector[j] << "):\n";
    outfile << "#Position"
            << "\t"
...
// Function to print Top N sum and their frequences agains total oligo number in
...
void map_class::TopN_sum_and_freq() {
  ...
  for (unsigned int i = 0; i < tot_sum_matrix.size(); i++) {
    ...
    if (DS && ordering == "p") {
      ...
      outfile.open(to_string(kmers_vector[i]) + "-mers_Top" + to_string(top_N) +
                   "_sum_and_frequence_SS_p_val.txt");
    }

    else {
      outfile.open(to_string(kmers_vector[i]) + "-mers_Top" + to_string(top_N) +
                   "_sum_and_frequence_SS_occ.txt");
    }

    outfile << "###Top " << top_N
            << " occurrences sum with k = " << kmers_vector[i] << ":\n";
    outfile << "Position"
            << "\t"
            << "Sum"
            << "\t"
    ...
    for (unsigned int j = 0; j < tot_sum_matrix[i].size(); j++) {
      ...
      double sum =
          tot_sum_matrix[i][j]; // Put as double to dont loose the precision
      double frequence = sum / tot_freq_matrix[i][j];
      outfile << j + 1 << "\t" << tot_sum_matrix[i][j] << "\t" << frequence
              << endl;
    }
    outfile.close();
  }
...
// Function to print the parameters used to calculate p-values. For each oligo
// ranked all its parameters are visualized
void map_class::p_value_parameters_debug_p_val() {
  ...
  for (unsigned int j = 0; j < P_VALUE_MATRIX.size(); j++) {
    ...
    outfile << "#Parameters used to calculate p_value for each oligo "
               "positionally ranked\n";
    ...
    for (unsigned int i = 0; i < P_VALUE_MATRIX[j].size(); i++) {
      ...
      // Printing into the file the parameters used to calculate each oligo
      // p-value in the top N ranking
      for (multimap<double, pair<string, vector<unsigned int>>>::iterator it =
               P_VALUE_MATRIX[j][i].p_value_KNT.begin();
           it != P_VALUE_MATRIX[j][i].p_value_KNT.end() && rank < top_N;
           it++, rank++) {
...
void map_class::p_value_parameters_debug_occ() {
  ...
  for (unsigned int j = 0; j < P_VALUE_MATRIX.size(); j++) {
    ...
    for (unsigned int i = 0; i < P_VALUE_MATRIX[j].size(); i++) {
      ...
      for (multimap<pair<unsigned int, unsigned int>,
                    pair<string, string>>::reverse_iterator it_rev =
               P_VALUE_MATRIX[j][i].vertical_multimap.rbegin();
           it_rev != P_VALUE_MATRIX[j][i].vertical_multimap.rend() &&
           rank < top_N;
           it_rev++, rank++) {
...
// Function to open and print the header of hamming output file, following the
// analysis parameters
ofstream map_class::outfile_header_hamming(unsigned int j) {
  ...
  outfile << "#Position"
          ...
          << "Num_Occ_Best_oligo"
          << "\t"
          << "Humming_found_oligo_number"
          << "\t"
          << "Total_occurrences(best+hamming)"
          << "\t"
          << "FREQUENCE_1"
          << "\t"
          << "FREQUENCE_2\n";
...
void print_debug_Z_scores(ofstream &outfile, unsigned int j,
  ...
  for (unsigned int position = 0; position < Z_TEST_MATRIX[0].size();
       position++) {
