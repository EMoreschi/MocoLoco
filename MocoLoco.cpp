	...
	for(multimap<pair<unsigned int, unsigned int>, pair<string,string>>::iterator it = vertical_multimap.begin(); it != vertical_multimap.end(); it++)
	{
		...
		for (unsigned int k = 0; k < 6; k++){
			...
			switch(oligo_vertical[k]){
				...
				case 'A':			
					P_oligo = P_oligo * PWM_hamming[0][k];
					break;

				case 'C':
					P_oligo = P_oligo * PWM_hamming[1][k];
					break;

				case 'G':
					P_oligo = P_oligo * PWM_hamming[2][k];
					break;

				case 'T':
					P_oligo = P_oligo * PWM_hamming[3][k];
					break;
...
//void likelihood_ratio(vector<vector<double>> PPM_em){
...
/////DEBUG/////////////////////////////////////////////////////////
unordered_map<string, unsigned int> map_class::return_horizontal_map_plus(){

	return orizzontal_plus;
}
	...
	for(unsigned int j=0; j<kmers_vector.size(); j++){
		...
		if(DS == 1){
			...
			for(unsigned int position = 0; position < Z_TEST_MATRIX[j].size(); position++){
				...
				cout << "#Position " << local_pos << ": \n#PWM calculated from oligo " << best_oligo << endl << endl;
				for(unsigned int i = 0; i< PWM_hamming.size(); i++){
...
void map_class::print_debug_Z_scores(ofstream& outfile, unsigned int j, unsigned int k){
	...
	outfile << "#Position" << "\t" << "best_oligo" << "\t" << "Local_mean" << "\t" << "Global_mean" << "\t" << "Local_std_dev" << "\t" << "Global_std_dev" << "\t" << "Z_score" << "\t" << "P-value" << "\t" << "P-value_Log10" << "\t" << "Bonferroni P-value" << "\t" << "Bonferroni_Log10" << endl;
	...
	for(unsigned int position = 0; position < Z_TEST_MATRIX[j].size(); position++){
		...
		double global_dev_std = Z_TEST_MATRIX[j][position].return_global_std_dev();
		double Zpvalue  = Z_TEST_MATRIX[j][position].return_Zpvalue();
		double Zpvalue_Log10  = abs(log10(Z_TEST_MATRIX[j][position].return_Zpvalue()));
		double z_score  = Z_TEST_MATRIX[j][position].return_z_score();
		double bonferroni = Zpvalue * ((half_length*2) - k);
		double bonferroni_Log10 = abs(log10(bonferroni));

		best_oligo = HAMMING_MATRIX[j][local_pos-1].return_real_best_oligo();	

		outfile << local_pos << "\t" << best_oligo << "\t" << local_mean << "\t" << global_mean << "\t" << local_dev_std << "\t" << global_dev_std << "\t" << z_score << "\t" << Zpvalue << "\t" << Zpvalue_Log10 << "\t" << bonferroni << "\t" << bonferroni_Log10 << endl;
	}
}
/*
...
void command_line_parser(int argc, char** argv){
	...
	const char* const short_opts = "hp:k:b:j:m:d:o:f:art:n:e:s";
...
void display_help(){
	...
	cerr << "\n --exp_maximization || -e to refine PWM matrices with the expectation maximization method" << endl;
	cerr << endl;
