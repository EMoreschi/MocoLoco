...
void map_class::print_debug_PWM_hamming_tomtom(ofstream &outfile,
  ...
  for (unsigned int position = 0; position < Z_TEST_MATRIX[j].size();
    ...
    PWM_hamming =
        HAMMING_MATRIX[j][Z_TEST_MATRIX[j][position].local_pos - 1].PWM_hamming;
...
void map_class::print_debug_PWM_hamming(ofstream &outfile, unsigned int j,
  ...
  for (unsigned int position = 0; position < Z_TEST_MATRIX[j].size();
    ...
    outfile << "#Position " << Z_TEST_MATRIX[j][position].local_pos
            ...
            << HAMMING_MATRIX[j][Z_TEST_MATRIX[j][position].local_pos - 1]
                   .real_best_oligo
            << " and his "
            << HAMMING_MATRIX[j][Z_TEST_MATRIX[j][position].local_pos - 1]
                       .similar_oligos.size() -
...
// PWM_matrices, parameters to calculate z-score, z-score and p-value printing
void map_class::print_debug_Z_scores(ofstream &outfile, unsigned int j,
                                     unsigned int k) {
  // PROFILE_FUNCTION();
