...
for(int i = 1; i < argc; i++){
	...
	int is_parameter = stoi(buf);			//convert argument into integer and put in variable is_parameter
	string input_type = typeid(is_parameter).name(); //put the type of is_parameter variable into a string called input_type
	if(input_type == "i"){				//if input_type is "i" means that the current argument buf is an integer

			parameter = stoi(argv[i]);      //put the argument value (converted to integer) in parameter variable
